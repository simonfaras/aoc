...
const calc1 = (input) => {
  ...
  const getContainerColors = (contentColor) => {
    return Array.from(
      Object.entries(input).reduce((list, [color, rules]) => {
  ...
  while (checkColors.length) {
    ...
    const newColors = contentColors.filter(
      (color) => !checkedColors.has(color)
    );

    checkColors = [...checkColors, ...newColors];

    checkColors.concat(
      contentColors.filter((color) => !checkedColors.has(color))
    );
  }
...
const calc2 = () => {
  ...
  const getContentCount = (color, multiplier) => {
    ...
    const temp =
      ...
      rules.reduce(
        (recursiveTotal, rule) =>
          recursiveTotal + getContentCount(rule.color, rule.count),
        0
      ) *
        multiplier;
